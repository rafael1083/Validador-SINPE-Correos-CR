...
test.describe('Validador SINPE - UI Tests', () => {
  ...
  test('QA-12: Multichunches muestra Michelle (3500) y Anier (8800)', async ({ page }) => {
    ...
    const michelleMonto = page.locator('text=3500.00');
    ...
    await expect(michelleName).toBeVisible();
    await expect(michelleRef).toBeVisible();
    await expect(michelleMonto).toBeVisible();

    // Verificar Anier: monto 8800.00, referencia, nombre
    const anierMonto = page.locator('text=8800.00');
    const anierRef = page.locator('text=2026042115183010901020165');
    const anierName = page.locator('text=Anier Gonzalez');

    await expect(anierName).toBeVisible();
    await expect(anierRef).toBeVisible();
    await expect(anierMonto).toBeVisible();
  ...
  test('QA-13: Filtro Esta Semana funciona', async ({ page }) => {
    ...
    // Michelle y Anier están en esta semana (hoy 2026-04-21)
  ...
  test('QA-14: Descarga CSV formato correcto', async ({ page, context }) => {
    ...
    // Verificar líneas limpias sin corrupción
    const lines = content.split('\n').filter(l => l.trim());
    expect(lines.length).toBeGreaterThan(1);

    // Michelle y Anier en CSV
    expect(content).toContain('2026042115183010500934322');
    expect(content).toContain('Michelle');
    expect(content).toContain('2026042115183010901020165');
    expect(content).toContain('Anier');

    console.log(`✅ CSV descargado: ${download.suggestedFilename()} (${lines.length} líneas)`);
  });
